...
- [新功能] 新增 `--alert` 告警模式：跳过情报搜索和 AI 分析，仅基于均线排列/价格突破MA5/涨跌幅/量比四类规则触发提醒并通过现有通知渠道推送；支持 `ALERT_CHANGE_PCT_THRESHOLD`、`ALERT_VOLUME_RATIO_THRESHOLD`、`ALERT_CHECK_MA_ALIGNMENT`、`ALERT_CHECK_PRICE_VS_MA5` 四个阈值配置项。
